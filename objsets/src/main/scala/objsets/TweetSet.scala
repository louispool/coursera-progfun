...
abstract class TweetSet {
  ...
  def descendingByRetweet: TweetList = {
    def loop(l: TweetList, s: TweetSet): TweetList = {
      if (s.isEmpty) l
      else {
        val t = s.mostRetweeted
        loop(new Cons(t, l), s.remove(t))
      }
    }
...
object Nil extends TweetList {
  ...
  def reverse = throw new java.util.NoSuchElementException("reverse of EmptyList")
}
...
class Cons(val head: Tweet, val tail: TweetList) extends TweetList {
  ...
  def reverse: TweetList = {
    def loop(in: TweetList, out: TweetList): TweetList = {
      if (in.isEmpty) out
