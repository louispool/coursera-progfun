...
object Huffman {
  ...
  def chars(tree: CodeTree): List[Char] = tree match {
    case Fork (left: CodeTree, right: CodeTree, ch: List[Char], weight: Int) => chars(left) ::: chars(right)
    case Leaf (ch: Char, weight: Int) => List(ch)
  }
  ...
  def times(chars: List[Char]): List[(Char, Int)] = chars match {
    case Nil => Nil
    case x :: xs => List((x, count(x, xs, 1))) ::: times(chars.filter(elem => elem != x))
  }

  def count(c: Char, chars: List[Char], cnt: Int): Int = chars match {
    case Nil => cnt
    case x :: xs => if (x == c) count(c, xs, cnt + 1) else count(c, xs, cnt)
  }
  ...
  /**
   * Inserts a tree into a list at a position such that the ordering by weights is preserved
   */
  def insert[T <: CodeTree](branch: T, xs: List[T]): List[T] = xs match {
    case Nil => List(branch)
    case y :: ys => if (branch.weight <= y.weight) branch :: xs else y :: insert(branch, ys)
  }
  ...
  def singleton(trees: List[CodeTree]): Boolean = trees match {
    case Nil => false //throw new Exception("Empty list")
    case x :: xs => xs == Nil
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    ...
    def loop(branch: CodeTree, bits: List[Bit], chars: List[Char]): List[Char] = {
      if (bits.isEmpty) {
        branch match {
          case Leaf(ch: Char, _) => chars ::: List(ch)
          case _ => chars
        }
      } else {
        branch match {
          case Leaf(ch: Char, _) => loop(tree, bits, chars ::: List(ch))
          case Fork(left: CodeTree, right: CodeTree, _, _) => if (bits.head == 0) loop(left, bits.tail, chars) else loop(right, bits.tail, chars)
        }
      }
    }
    loop(tree, bits, List())
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    def loop(chars: List[Char], bits: List[Bit]): List[Bit] = chars match {
      case Nil => bits
      case x :: xs => loop(xs, traverse(tree, x, bits))
    }
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] = table match {
    case Nil => throw new Exception("Character not in table")
