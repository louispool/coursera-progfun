...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.sorted.groupBy(x => x).mapValues(str => str.length).toList.sorted
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    occurrences.foldRight(List[Occurrences](List())) ((occurrence, accum) => {
      (for {
        combo <- accum
        num <- 1 to occurrence._2
      } yield (occurrence._1, num) :: combo) ++ accum
    })
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    y.foldLeft(x.toMap) ((map, occurrence) => {
      val freq = map(occurrence._1) - occurrence._2
      if (freq <= 0)
        map.-(occurrence._1)
      else
        map.updated(occurrence._1, freq)
    }).toList.sorted
  }
